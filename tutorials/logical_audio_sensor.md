...
As we can see, we use a `<source>` tag to define the source.
An explanation of all of the tags can be found in the [plugin documentation](https://github.com/ignitionrobotics/ign-gazebo/blob/1eb4253c6283e711e983be2b398bc7c0411c9408/src/systems/logical_audio_sensor_plugin/LogicalAudioSensorPlugin.hh#L45-L86), but there are a few important things to point out:
...
You can also take a look at the [microphone documentation](https://github.com/ignitionrobotics/ign-gazebo/blob/1eb4253c6283e711e983be2b398bc7c0411c9408/src/systems/logical_audio_sensor_plugin/LogicalAudioSensorPlugin.hh#L88-L109) for a detailed explanation of the tags embedded in the `<microphone>` tag.
...
One other important thing to note is that calling the play service on a source that is already playing does nothing - so, if the source attached to `red_box` is already playing, and you call the play service on it, it will not play for another 10 seconds - the source will still stop at the original 10 second duration mark.
